...
void Camera::process_mouse_movement(float x_offset, float y_offset, bool constrain_pitch)
{
    ...
    // Ograniczamy ruch w pionie, aby uniknąć "przewrócenia się" kamery
    if (constrain_pitch)
    {
        if (m_pitch > 89.0f)
            m_pitch = 89.0f;
...
{
    // Oblicz nowy wektor "front" na podstawie kątów Yaw i Pitch
    glm::vec3 front;
    ...
    // Normalizujemy, bo długość wektora może się minimalnie zmieniać przez niedokładności zmiennoprzecinkowe
    m_right = glm::normalize(glm::cross(m_front, m_world_up));
    m_up = glm::normalize(glm::cross(m_right, m_front));

    // Zaktualizuj macierz widoku
